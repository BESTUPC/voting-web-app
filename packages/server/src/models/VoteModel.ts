...
export class VoteModel {
     ...
     */
    public static async addOrUpdateVote({ userId, pollId, option }: IVote): Promise<boolean> {
    ...
    public static async deleteByPollId(pollId: string): Promise<boolean> {
        const updateCount: number = (await VoteModel._getCollection().deleteMany({ pollId }))
            .deletedCount;
