...
export class PollModel {
     ...
     */
    public static async getAll(membership: Array<EMembership>): Promise<Array<IPoll>> {
     ...
     */
    public static async setState(_id: ObjectId, state: EPollState): Promise<boolean> {
        const updateCount: number = (
            await PollModel._getCollection().updateOne({ _id }, { $set: { state } })
        ).modifiedCount;
