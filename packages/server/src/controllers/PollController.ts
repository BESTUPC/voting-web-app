...
export class PollController {
     ...
     * @throws Error 404 if the user or poll is not found.
     */
    public static async updateState(userId: string, _id: string): Promise<boolean> {
     ...
     * @throws Error 404 if the user or poll is not found.
     */
    public static async addPoll(userId: string, body: unknown): Promise<boolean> {
        if (await UserController.isAdmin(userId)) {
            const poll: IPoll = {
     ...
     * @throws Error 404 if the user or poll is not found.
     */
    public static async deletePoll(userId: string, _id: string): Promise<boolean> {
        if (await UserController.isAdmin(userId)) {
            const poll: IPoll = await PollModel.get(new ObjectId(_id));
            if (!poll) throw new ErrorHandler(404, `Poll ${_id} not found.`);
